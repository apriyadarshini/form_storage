...
def test_display_args1(client):
    assert client.get(url_for('forms.display'),data=json.dumps(dict(value="xyzsda"))).status_code == 200
    
    
def test_display_args2(client):
    assert client.get(url_for('forms.display'),data=json.dumps(dict(localisable="Yes"))).status_code == 200
